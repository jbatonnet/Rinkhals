...
def init():
    ...
    if system == 'Windows':
        ...
        rot = lv.DISPLAY_ROTATION_270
        disp.set_rotation(rot)

        touch = lv.evdev_create(lv.INDEV_TYPE_POINTER, '/dev/input/event0')
        touch.set_display(disp)
...
def main():
    ...
    minbtn.align(lv.ALIGN_TOP_LEFT, 0, 0)
    minp = lv.label(minbtn)
    minp.set_text('0,0')

    maxbtn = lv.button(scr)
    maxbtn.align(lv.ALIGN_BOTTOM_RIGHT, 0, 0)
