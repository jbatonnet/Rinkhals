...
class Firmware:
    ...
    def get_latest_version():
        try:
            ...
            if not result:
                ...
                version.version = Firmware.get_current_version()
                version.date = None
    ...
    def get_available_versions(limit=10):
        ...
        try:
            ...
            for entry in manifest.get('firmwares', [])[:limit]:
...
class Diagnostic:
    ...
    def __init__(self, type, short_text, long_text, icon=None, fix=None):
        self.type = type
    ...
    def collect():
        ...
        if not printer_info:
            yield Diagnostic(
                ...
                fix=DiagnosticFixes.REINSTALL_FIRMWARE
            )
            ...
            try:
                ...
                if len(custom_lines) > 0:
                    yield Diagnostic(
                        ...
                        fix=DiagnosticFixes.RESET_CONFIGURATION
                    )
            except:
                pass
        ...
        if USING_SIMULATOR:
            ...
            yield Diagnostic(
                ...
                long_text='You have printer configuration customizations',
                fix=DiagnosticFixes.REINSTALL_RINKHALS
            )
