...
# After MD5: f35882666c7b304d5737445c78975ce4

TARGET=$1

MD5=$(md5sum $TARGET | awk '{print $1}')
if [ "$MD5" = "f35882666c7b304d5737445c78975ce4" ]; then
...
dd if=$PATCH_FILE skip=323 ibs=1 of=$TARGET seek=966436 obs=1 count=4 conv=notrunc # 0xebf24 / 0xfbf24 > 0x000090e5
dd if=$PATCH_FILE skip=327 ibs=1 of=$TARGET seek=966444 obs=1 count=8 conv=notrunc # 0xebf2c / 0xfbf2c > 0xaef9feeb03fcffea
dd if=$PATCH_FILE skip=335 ibs=1 of=$TARGET seek=2782252 obs=1 count=16 conv=notrunc # 0x2a742c / 0x2b742c > 0x52696e6b68616c730052696e6b68616c
